...
CREATE TRIGGER update_client_documents_updatedat 
...
SELECT 
    ...
    COUNT(CASE WHEN documenttype = 'identity_front' THEN 1 END) as identity_front_count,
    COUNT(CASE WHEN documenttype = 'identity_back' THEN 1 END) as identity_back_count,
    COUNT(CASE WHEN documenttype = 'insurance_contract' THEN 1 END) as insurance_contract_count,
    COUNT(CASE WHEN documenttype = 'proof_address' THEN 1 END) as proof_address_count,
    COUNT(CASE WHEN documenttype = 'bank_statement' THEN 1 END) as bank_statement_count,
    COUNT(CASE WHEN documenttype = 'additional' THEN 1 END) as additional_count,
    -- Vérifier si tous les documents obligatoires sont présents
    CASE 
        WHEN COUNT(CASE WHEN documenttype = 'identity_front' THEN 1 END) > 0
         AND COUNT(CASE WHEN documenttype = 'identity_back' THEN 1 END) > 0
         AND COUNT(CASE WHEN documenttype = 'insurance_contract' THEN 1 END) > 0
        THEN true 
        ELSE false 
...
GROUP BY clientid, token;

-- Fonction pour obtenir le résumé des documents d'un client
CREATE OR REPLACE FUNCTION get_client_documents_summary(p_clientid VARCHAR, p_token VARCHAR)
RETURNS JSON AS $$
DECLARE
    result JSON;
BEGIN
    SELECT json_build_object(
        ...
        'required_documents_complete', (
            COUNT(CASE WHEN documenttype = 'identity_front' THEN 1 END) > 0
            AND COUNT(CASE WHEN documenttype = 'identity_back' THEN 1 END) > 0
            AND COUNT(CASE WHEN documenttype = 'insurance_contract' THEN 1 END) > 0
        ),
        ...
        'total_size_mb', ROUND(SUM(filesize) / 1024.0 / 1024.0, 2),
        'first_upload', MIN(uploaddate),
    ...
    FROM (
        SELECT 
            ...
            uploaddate
        FROM client_documents 
        WHERE clientid = p_clientid AND token = p_token
        GROUP BY documenttype, filesize, uploaddate
    ) grouped;
...
INSERT INTO client_documents (clientid, token, documenttype, filename, filepath, filesize, mimetype) VALUES
...
('a71df0e4-e48a-4fec-bdd3-a2780daf7bcd', 'a71df0e4-e48a-4fec-bdd3-a2780daf7bcd', 'insurance_contract', 'contrat.pdf', '/uploads/clients/a71df0e4-e48a-4fec-bdd3-a2780daf7bcd/insurance_contract/contrat.pdf', 2048000, 'application/pdf');

-- Vérifier que tout fonctionne
SELECT 'Table client_documents créée avec succès!' as message;

-- Tester la vue des statistiques
SELECT * FROM client_documents_stats WHERE clientid = 'a71df0e4-e48a-4fec-bdd3-a2780daf7bcd';
